local CODE_VERSION = 1
local START_CHARACTER = "<"
local LAST_CHARACTER = ">"
local SPLIT_CHARACTER = ":"

local alphabet =
    'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'

local colorIds = {
    -- Button
    "button_highlight", --
    "button_background", --
    "button_shadow", --
    "button_selected", --
    -- Tab
    "tab_corner_highlight", --
    "tab_highlight", --
    "tab_background", --
    "tab_shadow", --
    -- Window
    "window_hover", --
    "window_highlight", --
    "window_background", --
    "window_shadow", --
    "window_corner_shadow", --
    -- Text
    "text_regular", --
    "text_active", --
    "text_link", --
    "text_separator", --
    -- Field
    "field_highlight", --
    "field_background", --
    "field_shadow", --
    "field_corner_shadow", --
    -- Editor
    "editor_background", --
    "editor_background_shadow", --
    "editor_tooltip", --
    "editor_tooltip_shadow", --
    "editor_tooltip_corner_shadow", --
    "editor_cursor", --
    "editor_cursor_shadow", --
    "editor_cursor_outline", --
    "editor_icons" --
}

local function NumberToBinaryString(x, legth)
    local result = ""
    while x ~= 1 and x ~= 0 do
        result = tostring(x % 2) .. result
        x = math.modf(x / 2)
    end
    result = tostring(x) .. result

    while legth and #result < legth do result = "0" .. result end

    return result
end

local function NumberToBase64(x)
    if x == 0 then return alphabet:sub(1, 1) end

    local remainder = x
    local result = ""

    while remainder > 0 do
        local part = remainder % 64
        remainder = (remainder - part) / 64

        result = alphabet:sub(part + 1, part + 1) .. result
    end

    return result
end

local function Base64ToNumber(x)
    local result = 0

    for i = 1, #x do
        local character = x:sub(i, i)
        local xyz = string.find(alphabet, character) - 1

        result = result + (xyz * (64 ^ (#x - i)))
    end

    return math.floor(result)
end

local function ColorToRgbBinary(color)
    return NumberToBinaryString(color.red, 8) ..
               NumberToBinaryString(color.green, 8) ..
               NumberToBinaryString(color.blue, 8)
end

local function RgbBinaryToColor(n)
    return Color {
        red = tonumber(n:sub(1, 8), 2),
        green = tonumber(n:sub(9, 16), 2),
        blue = tonumber(n:sub(17, 24), 2),
        alpha = 255
    }
end

local function EncodeColor(color)
    local rgbBinary = ColorToRgbBinary(color)
    local rgbNumber = tonumber(rgbBinary, 2)
    local rgbEncoded = NumberToBase64(rgbNumber)

    while #rgbEncoded < 4 do rgbEncoded = alphabet:sub(1, 1) .. rgbEncoded end

    return rgbEncoded
end

local function DecodeColor(encodedColor)
    local rgbNumber = math.floor(Base64ToNumber(encodedColor))
    local rgbBinary = NumberToBinaryString(rgbNumber, 24)
    local decodedColor = RgbBinaryToColor(rgbBinary)

    return decodedColor
end

local function EncodeParameters(parameters)
    local advancedFlag = parameters.isAdvanced and "1" or "0"

    local parametersBinary = advancedFlag
    local parametersNumber = tonumber(parametersBinary, 2)
    local result = NumberToBase64(parametersNumber)

    return result
end

local function EncodeColors(colors)
    local result = ""

    for _, id in ipairs(colorIds) do
        result = result .. EncodeColor(colors[id])
    end

    return result
end

local function EncodeSigned(name, parameters, colors)
    -- Validate name
    if #name == 0 or #name > 32 then return end

    local encodedParameters = EncodeParameters(parameters)
    local encodedColors = EncodeColors(colors)

    -- <THEME_NAME:CODE_VERSION:ENCODED_PARAMS:ENCODED_COLORS>
    return START_CHARACTER .. name .. SPLIT_CHARACTER .. CODE_VERSION ..
               SPLIT_CHARACTER .. encodedParameters .. SPLIT_CHARACTER ..
               encodedColors .. LAST_CHARACTER
end

local function SplitString(string, separator)
    local result = {}

    local partial = ""
    for i = 1, #string do
        local c = string:sub(i, i)
        if c ~= separator then
            partial = partial .. c
        else
            table.insert(result, partial)
            partial = ""
        end
    end

    if #partial > 0 then table.insert(result, partial) end

    return result
end

local function DecodeParameters(version, encodedParameters)
    -- FUTURE: Update parameters depending on the version
    _ = version

    -- FUTURE: This will depend on version
    local expectedBinaryLength = 1

    local parametersNumber = Base64ToNumber(encodedParameters)
    local parametersBinary = NumberToBinaryString(parametersNumber,
                                                  expectedBinaryLength)

    local advancedFlag = parametersBinary:sub(1, 1)

    local parameters = {isAdvanced = advancedFlag == "1"}

    return parameters
end

local function DecodeColors(version, encodedColors)
    -- FUTURE: Update colors depending on the version
    _ = version

    local decodedColors = {}

    for i = 1, #encodedColors, 4 do
        local encodedColor = encodedColors:sub(i, i + 3)
        table.insert(decodedColors, DecodeColor(encodedColor))
    end

    local colors = {}
    for i, id in ipairs(colorIds) do colors[id] = decodedColors[i] end

    return colors
end

local function DecodeSigned(code)
    if code == nil or #code == 0 then return end

    local startIndex = code:find(START_CHARACTER)
    if startIndex == nil then return end

    local lastIndex = code:find(LAST_CHARACTER)
    if lastIndex == nil then return end

    local trimmedSignedCode = code:sub(startIndex + 1, lastIndex - 1)
    local codeSplit = SplitString(trimmedSignedCode, SPLIT_CHARACTER)
    if #codeSplit ~= 4 then return nil end

    local name = codeSplit[1]
    local version = codeSplit[2]
    local encodedParams = codeSplit[3]
    local encodedColors = codeSplit[4]

    local parameters = DecodeParameters(version, encodedParams)
    local colors = DecodeColors(version, encodedColors)

    return {name = name, parameters = parameters, colors = colors}
end

local function DecodeName(code)
    if code == nil or #code == 0 then return end

    local startIndex = code:find(START_CHARACTER)
    if startIndex == nil then return end

    local lastIndex = code:find(LAST_CHARACTER)
    if lastIndex == nil then return end

    local trimmedSignedCode = code:sub(startIndex + 1, lastIndex - 1)
    local codeSplit = SplitString(trimmedSignedCode, SPLIT_CHARACTER)
    if #codeSplit ~= 4 then return nil end

    local name = codeSplit[1]

    return name
end

return {
    EncodeSigned = EncodeSigned,
    DecodeSigned = DecodeSigned,
    DecodeName = DecodeName
}
