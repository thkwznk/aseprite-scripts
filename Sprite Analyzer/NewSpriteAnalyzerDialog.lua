...
function SpriteAnalyzerDialog:Create(options)
    ...
    :combobox{
        ...
        onchange = function() self:Repaint() end
    } --
...
function SpriteAnalyzerDialog:Repaint(image)
    if image then self.previewImage = image end
    if self.dialog then self.dialog:repaint() end
end

function SpriteAnalyzerDialog:GetAnalysisMode()
    return self.dialog and self.dialog.data.analysisMode
end
